 ...
 *
 */
public class ExperimenterOneParamVariedWindow extends JFrame implements ThreadCompleteListener, UncaughtExceptionHandler {
	/** serial UID */
	 ...
	 * Constructor
	 */
	public ExperimenterOneParamVariedWindow() {
		...
		buttonHelp.setIcon(new ImageIcon(ExperimenterOneParamVariedWindow.class.getResource("/ca/pfv/spmf/gui/icons/About24.gif")));
				...
				for(int i = 0; i<description.length; i++) {
					...
					if(parameter.isOptional == false) {
						if(i < newDescription.length) {
							...
							if(parameter.getParameterType() != newParameter.getParameterType() || !parameter.name.equals(newParameter.name)) {
								sameMandatoryParameters = false;
							}
						}else {
								sameMandatoryParameters = false;
...
//			
	...
	private void askUserToChooseInputFile() {
		try {
			...
			int returnVal = fc.showSaveDialog(ExperimenterOneParamVariedWindow.this);
