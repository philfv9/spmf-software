 ...
 */
/**
 * This class executes commands from the command line interface or graphical
 * interface to run the algorithms.
 * 
...
public class CommandProcessor {
	...
	/**
	 * This method run an algorithm. It is called from the GUI interface or when the
	 * user run the jar file from the command line.
	 * 
	 ...
	 */
	public static void runAlgorithm(String algorithmName, String inputFile, String outputFile, String[] parameters)
			throws Exception {
		...
		if (algorithm.getInputFileTypes() != null) {
			if (inputFile == null) {
				throw new IllegalArgumentException(
						System.lineSeparator() + System.lineSeparator() + "No input file has been provided.");
			}

			File input = new File(inputFile);
			if (input.exists() == false) {
				throw new IllegalArgumentException(System.lineSeparator() + System.lineSeparator() + " The input file '"
		...
		if (algorithm.getOutputFileTypes() != null && outputFile == null) {
			throw new IllegalArgumentException(
					System.lineSeparator() + System.lineSeparator() + " No output file path has been provided.");
		}
		...
		for (int i = 0; i < numberOfParameter; i++) {
			...
			// If it is the command line interface and the user has not provided the
			// parameter i
			if (i == parameters.length) {
				...
				// If the parameter is null but it is not optional, then we need to inform the
				// user
		...
		if (algorithmName.startsWith("Convert") == false 
				&& AlgorithmType.DATA_VIEWER.equals(algorithm.getAlgorithmType()) == false
				&& inputFile != null) {
			...
			if (firstLine.startsWith("@CONVERTED_FROM_ARFF") || firstLine.startsWith("@CONVERTED_FROM_TEXT")
					...
					|| firstLine.startsWith("@CONVERTED_FROM_GPATTERNS")) {
				...
				if ((inputFile.endsWith(".arff") || inputFile.endsWith(".ARFF"))) {
					...
					outputFile = outputFile + ".tmp";
					mapItemToString = converter.convertARFFandReturnMap(originalInputFile, inputFile,
							Integer.MAX_VALUE);
		...
		if (algorithm.getOutputFileTypes() != null && mapItemToString != null) {
