...
*/
/**
 * HMP_HC_Runner implements a Hill Climbing-based Heuristic Mining Pattern
 * algorithm for data compression optimization. The algorithm discovers frequent
 * patterns that best compress transactional databases using local search
 * optimization.
 * 
 * @author Chen Enze, M. Zohaib Nawaz, Philippe Fournier-Viger, M. Saqib Nawaz
 */
public class AlgoHMPHC {
	...
	/** if this variable set to true, some debugging information is shown */
	final boolean DEBUG_MODE = false;

	// ================= Optional parameters
	/** Maximum iterations per pattern optimization in hill climbing */
	int max_iterations = 31;
	 ...
	 */
	public ArrayList<int[]> runAlgorithm(String inputPath, String outputPath, int maxCodeTableSize) {
		...
		while (codetable.size() < maxCodeTableSize / 3) {
			...
			while (iterations < max_iterations) {
				...
				iterations++;
				iterationTryCount++;
				if (iterationTryCount > 20) { // If the pattern can not find good neighbors, we should move
												// to the next pattern.
					break;
				}
			}
			...
			if (attemptsWithoutImprovement > 100 && attemptsWithoutImprovement > maxCodeTableSize / 3) {
				break;
			}
		}
...
//			itemsetCount.put(pattern, count);
		...
		writePatternsToFile(outputPath);
	...
	/**
	 * Set the maximum number of iterations (default = 31)
	 * 
	...
	private void writePatternsToFile(String outputPath) {
		try (BufferedWriter writer = new BufferedWriter(new FileWriter(outputPath))) {
			...
			for (int[] pattern : codetable) {
				int support = itemsetCount.get(new PatternKey(pattern));
	 ...
	 */
	/**
	 * Calculates the uncompressed size in bits of a transactional database. This
	 * method provides the baseline compression size before pattern application.
	 * 
...
//		int patCounter = 0;
	 ...
	 */
	/**
	 * Reads a transactional database from a text file. Each line represents a
	 * transaction with space-separated integer items. Transactions are
	 * automatically sorted for consistent processing.
	 * 
	 ...
	 * (if not present) or removes them (if present), creating pattern diversity for
	 * the local search optimization.
	 * 
	 * @param currentPattern Base pattern for modification
	 * @return New pattern with two items added/removed, or null if pattern becomes
	 *         too small
	 */
	...
	public void printStats() {
		System.out.println("=============  HMP-HC - STATS =============");
