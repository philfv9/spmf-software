...
* 
* SPMF is free software: you can redistribute it and/or modify it under
...
public class AlgoEclat_Bitset {
	...
	public Itemsets runAlgorithm(String output, TransactionDatabase database, double minsupp,
		...
		loopJ:		for(int j=i+1; j < frequentItems.size(); j++) {
				...
				if(equivalenceClassIitems.size()>0) {
					...
					processEquivalenceClass(itemsetBuffer, 1, tidsetI.support,equivalenceClassIitems, equivalenceClassItidsets);
				}
			}
		}
	 ...
	 */
	private void processEquivalenceClass(int[] prefix, int prefixLength, int prefixSupport, List<Integer> equivalenceClassItems,
		...
		// If all tidsets in this equivalence class are identical (same support and same bitset),
		// then we can output all combinations of these suffix items without further intersections.
		boolean allSameSupportAsPrefix = true;
		for (int k = 0; k < equivalenceClassTidsets.size(); k++) {
		    BitSetSupport tk = equivalenceClassTidsets.get(k);
		    if (tk.support != prefixSupport) {
		        allSameSupportAsPrefix = false;
		        break;
		    }
		}
		if (allSameSupportAsPrefix) {
		    saveAllCombinationsEclat(prefix, prefixLength, equivalenceClassItems,
		            equivalenceClassTidsets.get(0));
		    return;
		}
		// ---------------------------------------------------------------------------
...
//		insertionSort(equivalenceClassItems, equivalenceClassTidsets);
		...
		for(int i=0; i< equivalenceClassItems.size(); i++) {
			...
			if(prefixLength+2 <= maxItemsetSize){
				...
				if(equivalenceClassISuffixItems.size() >0) {
					...
					// Recursive call
					processEquivalenceClass(prefix, newPrefixLength, tidsetI.support, equivalenceClassISuffixItems, equivalenceITidsets);
				}
			}
		}
	 ...
	 */
	protected void save(int[] prefix, int prefixLength, int suffixItem, BitSetSupport tidset) throws IOException {
	 ...
	 * @throws IOException if an error occurrs when writing to disk.
	 */
	protected void saveSingleItem(int item, int support, BitSet tidset) throws IOException {
		// increase the itemset count
		itemsetCount++;
		// if the result should be saved to memory
		if(writer == null){
	...
	/**
	 * Save all combinations of items when all tidsets have identical tidsets/support.
	 * Enumerates all non-empty subsets of `items` and saves them directly using the common tidset.
	 */
	private void saveAllCombinationsEclat(int[] prefix, int prefixLength,
			List<Integer> items, BitSetSupport commonTidset) throws IOException {

		int n = items.size();
		// Use long mask: supports up to 63 items in the equivalence class (practical).
		// If n is large, this will have exponential cost anyway.
		long max = 1L << n;
		loop1:
		for (long mask = 1L; mask < max; mask++) {
			...
			// save itemset represented in prefix[0..newLen-1] with the known support/tids
			savePrefixDirect(prefix, newLen, commonTidset.support, commonTidset.bitset);
		}
	}

	/**
	 * Save an itemset that is already fully formed in prefix[] (length 'length').
	 */
	private void savePrefixDirect(int[] prefix, int length, int support, BitSet bitset) throws IOException {
		itemsetCount++;
		if (writer == null) {
			int[] itemsetArray = new int[length];
			System.arraycopy(prefix, 0, itemsetArray, 0, length);
			Itemset itemset = new Itemset(itemsetArray);
			itemset.setAbsoluteSupport(support);
			frequentItemsets.addItemset(itemset, length);
		} else {
			StringBuilder buffer = new StringBuilder();
