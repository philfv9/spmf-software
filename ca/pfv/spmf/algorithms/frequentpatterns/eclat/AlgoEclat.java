...
/**
 * This is a recent version of the ECLAT algorithm. It uses sets of integers to
 * represent tidsets.
 * 
 * Eclat was proposed by ZAKI (2000). <br/>
 * <br/>
 * 
 * See this article for details about ECLAT: <br/>
 * <br/>
 * 
 * Zaki, M. J. (2000). Scalable algorithms for association mining. Knowledge and
 * Data Engineering, IEEE Transactions on, 12(3), 372-390. <br/>
 * <br/>
 * 
...
public class AlgoEclat {
	 ...
	 * @param output                          an output file path for writing the
	 *                                        result or if null the result is saved
...
//		System.out.println(matrix.getSupportForItems(262, 362));
		...
		if (maxItemsetSize >= 2) {
			...
			for (int i = 0; i < frequentItems.size(); i++) {
				...
				if (equivalenceClassIitems.size() > 0) {
					...
					processEquivalenceClass(itemsetBuffer, 1, supportI, equivalenceClassIitems,
...
//	}
	...
	/**
	 * This method process all itemsets from an equivalence class to generate larger
	 * itemsets,
	 * 
	 * @param prefix                  a common prefix to all itemsets of the
	 *                                equivalence class
	 * @param supportPrefix           the support of the prefix (not used by eclat,
	 *                                but used by dEclat)
	 * @param equivalenceClassItems   a list of suffixes of itemsets in the current
	 *                                equivalence class.
	 * @param equivalenceClassTidsets a list of tidsets of itemsets of the current
	 *                                equivalence class.
	 * @param prefixLength            the prefix length
	 * @throws IOException if error while writting the output to file
	 */
	private void processEquivalenceClass(int[] prefix, int prefixLength, int supportPrefix,
			List<Integer> equivalenceClassItems, List<Set<Integer>> equivalenceClassTidsets) throws IOException {
		...
		// ------------- NEW OPTIMIZATION - 2025 - SINGLE PATH OPTIMIZATION ----------
		// If all tidsets in this equivalence class are identical (same support and same
		// bitset),
		// then we can output all combinations of these suffix items without further
		// intersections.
		boolean allSameSupportAsPrefix = true;
		for (int k = 0; k < equivalenceClassTidsets.size(); k++) {
			int support = equivalenceClassTidsets.get(k).size();
			if (support != supportPrefix) {
				allSameSupportAsPrefix = false;
				break;
			}
		}
		if (allSameSupportAsPrefix) {
			saveAllCombinationsEclat(prefix, prefixLength, equivalenceClassItems, equivalenceClassTidsets.get(0));
			return;
		}
		// ---------------------------------------------------------------------------
...
//		insertionSort(equivalenceClassItems, equivalenceClassTidsets);
		...
		for (int i = 0; i < equivalenceClassItems.size(); i++) {
			...
			if (prefixLength + 2 <= maxItemsetSize) {
				...
				if (equivalenceClassISuffixItems.size() > 0) {
					...
					// Recursive call
					processEquivalenceClass(prefix, newPrefixLength, supportI, equivalenceClassISuffixItems,
							equivalenceITidsets);
				}
			}
		}
	...
	/**
	 * Save all combinations of suffix items in the current equivalence class using
	 * the single-path optimization.
	 * 
	 * @param prefix       the current prefix
	 * @param prefixLength the length of the prefix
	 * @param suffixItems  the items in the equivalence class
	 * @param tidset       the tidset of all items (they all share the same tidset)
	 * @throws IOException if error while writing to file
	 */
	private void saveAllCombinationsEclat(int[] prefix, int prefixLength, List<Integer> suffixItems,
			Set<Integer> tidset) throws IOException {
		int n = suffixItems.size();
		long max = 1L << n;
		loop1: for (long mask = 1L; mask < max; mask++) {
			int newLen = prefixLength;
			for (int j = 0; j < n; j++) {
				if ((mask & (1L << j)) != 0L) {
					if (newLen == maxItemsetSize) {
						continue loop1;
					}
					prefix[newLen++] = suffixItems.get(j);
				}
			}
// Save the fully built itemset once
			save(prefix, newLen - 1, prefix[newLen - 1], tidset, tidset.size());
		}
	}
	 ...
	 * @param supportPrefix the support of the prefix (not used by Eclat, but used
	 *                      by dEclat).
	 ...
	 */
	protected void save(int[] prefix, int prefixLength, int suffixItem, Set<Integer> tidset, int support)
			throws IOException {
	...
	/**
	 * Save an itemset containing a single item to disk or memory (depending on what
	 * the user chose).
	 * 
	 ...
	 * @throws IOException if an error occurrs when writing to disk.
	 */
	protected void saveSingleItem(int item, Set<Integer> tidset, int support) throws IOException {
		// increase the itemset count
		itemsetCount++;
		// if the result should be saved to memory
		if (writer == null) {
	...
	public void printStats() {
		System.out.println("=============  ECLAT v2.65 - STATS =============");
