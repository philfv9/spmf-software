...
//		insertionSort(equivalenceClassItems, equivalenceClassTidsets);
	...
	/**
	 * Save all combinations of suffix items in the current equivalence class
	 * using the single-path optimization.
	...
	private void saveAllCombinationsEclat(int[] prefix, int prefixLength,
	                                      List<Integer> suffixItems, Set<Integer> tidset) throws IOException {
	    int n = suffixItems.size();
	    // Generate all non-empty combinations of suffixItems
	    int[] combo = new int[n];
	    for (int mask = 1; mask < (1 << n); mask++) { // from 1 to 2^n - 1
	        int comboLength = 0;
	        for (int i = 0; i < n; i++) {
	            if ((mask & (1 << i)) != 0) {
	                combo[comboLength++] = suffixItems.get(i);
	            }
	        }
	        // Save the combination with prefix
	        for (int j = 0; j < comboLength; j++) {
	            save(prefix, prefixLength, combo[j], tidset, tidset.size());
	        }
	        // For combinations of more than 1 item, save the whole combination
	        if (comboLength > 1) {
	            int[] fullItemset = new int[prefixLength + comboLength];
	            System.arraycopy(prefix, 0, fullItemset, 0, prefixLength);
	            System.arraycopy(combo, 0, fullItemset, prefixLength, comboLength);
	            // Save as a single Itemset
	            save(fullItemset, prefixLength + comboLength - 1, combo[comboLength - 1], tidset, tidset.size());
	        }
	    }
	}
	 ...
	 */
	protected void save(int[] prefix, int prefixLength, int suffixItem, Set<Integer> tidset, int support) throws IOException {
