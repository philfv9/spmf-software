 ...
 * means that the algorithm counts the support of itemsets by using transaction identifiers lists
 * (tid-lists) rather than scanning the database.
 * 
 * This is an optimized version using a hybrid tidset representation:
 * dense (long[] bitset) for high-support itemsets and sparse (sorted int[] list) 
 * for low-support itemsets.
 * 
 ...
 */
public class AlgoAprioriTIDInverse_FAST {
	...
	/** the maximum support threshold (absolute) */
	 ...
	 * Default constructor
	 */
	public AlgoAprioriTIDInverse_FAST() {
	}

	/**
	 * Run the AprioriTID-Inverse algorithm.
	 * 
	 * @param input  path to input file
	 * @param output path to output file
	 * @param minsup minimum support threshold as a fraction (e.g. 0.5)
	 * @param maxsup maximum support threshold as a fraction (e.g. 0.6)
	 *
	 ...
	 */
	public void runAlgorithm(String input, String output, double minsup, double maxsup) throws NumberFormatException, IOException {
		...
		List<ItemsetWithTID> level = scanDatabaseAndBuildLevel1(input, minsup, maxsup);
	...
	/**
	 * Scan the database, identify frequent 1-itemsets within [minsup, maxsup], and build level 1.
	 * 
	 * @param input  path to input file
	 * @param minsup minimum support as a fraction
	 * @param maxsup maximum support as a fraction
	 * @return sorted list of frequent 1-itemsets
	 * @throws IOException if an error occurs reading the file
	 */
	protected List<ItemsetWithTID> scanDatabaseAndBuildLevel1(String input, double minsup, double maxsup) throws IOException {
		tidcount = 0;
		...
		// Compute absolute minimum and maximum support
		this.minSuppRelative = (int) Math.ceil(minsup * tidcount);
		this.maxSuppRelative = (int) Math.ceil(maxsup * tidcount);

		// Build level 1 - THE ONLY BIG DIFFERENCE WITH APRIORI IS HERE... 
		// WE CHECK MAXSUP FOR ITEMSETS OF SIZE 1
		k = 1;
		...
		while (iterator.hasNext()) {
			...
			// Check both minsup AND maxsup for items of size 1
			if (cardinality >= minSuppRelative && cardinality <= maxSuppRelative && maxItemsetSize >= 1) {
	...
	protected List<ItemsetWithTID> generateCandidateSizeK(List<ItemsetWithTID> levelK_1) throws IOException {
	    ...
	    // List to store candidate itemsets of size k
			...
			while (groupStart < levelSize) {
				...
				for (int i = groupStart; i < groupEnd; i++) {
					...
					for (int j = i + 1; j < groupEnd; j++) {
						...
						// If the join of both itemsets is frequent
						if (cardinality >= minSuppRelative) {
							...
							// Add as candidate and save to file
							candidates.add(candidate);
							saveItemsetToFile(candidate);
						}
					}
	...
	public void printStats() {
		System.out.println("=============  APRIORI INVERSE TID FAST v.2.66- STATS =============");
		System.out.println(" Transactions count from database : " + tidcount);
		System.out.println(" Sporadic itemset count : " + itemsetCount);
